...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    number_of_words = len(typed)
    return (number_of_words / 5) * (60 / elapsed)
    # END PROBLEM 4
...
def shifty_shifts(start, goal, limit):
    ...
    if start == goal:
        return 0
    elif start == '' or goal == '' or limit < 0:
        return max(len(start), len(goal))
    elif start[0] != goal[0]:
        count += 1
        limit -= 1
    return count + shifty_shifts(start[1:], goal[1:], limit)
    # END PROBLEM 6
...
def report_progress(typed, prompt, user_id, send):
    ...
    for i in range(len(typed)):
        if typed[i] == prompt[i] and continuous:
            correct += 1
...
def fastest_words(game):
    ...
    for i in player_indices:
        ...
        for j in word_indices:
            min_time, quickest_player_index = float('inf'), 0
            for k in player_indices:
                curr_time_at_k = time(game, k, j)
                if curr_time_at_k < min_time:
                    quickest_player_index = k
                    min_time = curr_time_at_k
            if i == quickest_player_index:
                word_list.append(word_at(game, j))
        fast_words.append(word_list)
    return fast_words
