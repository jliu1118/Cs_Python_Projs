...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    total_point, is_one = 0, False
    while num_rolls:
        current_dice = dice()
        if current_dice == 1:
            is_one = True
        total_point += current_dice
        num_rolls -= 1
    if is_one == True:
        return 1
    return total_point
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        highest_value = running_high
        if who == 0:
            current_value = score0
        else:
            current_value = score1
        points_difference = current_value - last_score
        if points_difference > highest_value:
            print(points_difference, 'point(s)! The most yet for Player', who)
            highest_value = points_difference
        return announce_highest(who, current_value, highest_value)
    return say
    # END PROBLEM 7
...
def make_averaged(original_function, trials_count=1000):
    ...
    def average_value(*args):
        ...
        while i < trials_count:
            dice_number_total += original_function(*args)
            i += 1
        average = dice_number_total / trials_count
        return average
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    """
    # BEGIN PROBLEM 9
    i, max_average, highest_dice = 1, 0, 0
    current_average = make_averaged(roll_dice, trials_count)
    while i < 11:
        score = current_average(i, dice)
